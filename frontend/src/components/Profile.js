...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/bookings/${user.id}`, {
      ...
      .then(res => {
          if (res.status === 401 || res.status === 403) { logout(); navigate('/login'); throw new Error('Unauthorized'); }
          return res.json();
      })
  ...
  const handleCancelBooking = async (bookingId) => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/bookings/${bookingId}/cancel`, { 
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` }
      });
